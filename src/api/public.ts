...
export interface PublicApiMonitor {
  id: number;
  name: string;
  url: string;
...
export interface PublicApiResponse {
  success: boolean;
  data?: PublicApiMonitor[];
  error?: string;
  timestamp: number;
}

export async function fetchPublicMonitors(
  days: number = 30
  ...
  try {
    ...
    if (response.data.stat !== 'ok') {
      return {
        success: false,
        error: response.data.error?.message || 'API 请求失败',
        timestamp: Date.now(),
      };
    }
